...
r_pi = np.diagonal(np.matmul(policy, rewards.T))
# some of you came up with the equivalent method below
r_pi_2 = np.sum(policy * rewards, axis=1)
assert(np.array_equal(r_pi, r_pi_2))

print("Rewards Matrix for MRP w.r.t policy")
print(r_pi)  # uncomment to verify

p_pi = np.diagonal(np.matmul(policy, dynamics))
# The diagonal represents the summations over actions where the state of the policy equalled the state of the dynamics.
# the diagonal of the 3D (policy times dynamics) matric is a (16,) array for all the possible 16 s'

print("Policy versus Dyanmics P w.r.t policy")
print(policy)  # uncomment below to see why it works
print(p_pi)
print(p_pi.sum(axis=1))


v_star = np.matmul(np.linalg.inv(np.identity(states.shape[0]) - gamma * p_pi.T), r_pi)
